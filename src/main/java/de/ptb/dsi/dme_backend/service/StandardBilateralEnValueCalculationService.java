...
// Berechnung des bilateralen En-Wertes zwischen jedem Paar von Teilnehmern
        for (int i = 0; i < listParticipantMeasuredValue.size(); i++) {
            for (int j = i + 1; j < listParticipantMeasuredValue.size(); j++) {
                ...
                double ui = participant1.getSiReal().getExpandedUncertainty();
                double uj = participant2.getSiReal().getExpandedUncertainty();
//                double ui = participant1.getSiReal().getExpandedUncertainty().getUncertainty();
//                double uj = participant2.getSiReal().getExpandedUncertainty().getUncertainty();
